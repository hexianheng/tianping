...
if(userId == "" || token == ""){
    ...
    var jsId = $("#jsId").val();
    ajax("/User/getRoleOne",{"userId":userId,"token":token,"roleId":jsId},function(results){
        $("#roleName").val(results['data']['name']);
        var quanxian = "|"+JSON.parse(results["data"]['permission']).join('|')+"|";
        ajax("/User/getPermission",{"userId":userId,"token":token},function(result){

            var data = result['data'];
            var html = '';
            for(var i = 0; i < data.length; i++){
                if(quanxian.indexOf("|"+data[i].id+"|")>=0)
                {
                    html += '<div sid="'+data[i].id+'" style="padding:10px" pid="'+data[i].parentId+'"><input status="0" style="margin-right:3px" class="check" type="checkbox" name="checkbox" value="'+data[i].id+'" checked/>'+data[i].name+'</div>';
                }else{
...
$("#btn").click(function(){
    ...
    ajax("/User/updRole",{"userId":userId,"token":token,"roleName":roleName,"permissionStr":permissionStr,"roleId":roleId},function(result){
